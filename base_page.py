...
class OperationsHelper(BasePage):
    def enter_login(self, word=testdata['login']):
        login_field = self.find_element(locator['LOCATOR_LOGIN_FIELD'])
        login_field.clear()
        login_field.send_keys(word)

    def enter_pass(self, word=testdata['password']):
        pass_field = self.find_element(locator['LOCATOR_PASS_FIELD'])
        pass_field.clear()
        pass_field.send_keys(word)

    def click_login_button(self):
        self.find_element(locator['LOCATOR_LOGIN_BTN']).click()
