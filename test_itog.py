...
def test_successful_login(browser):
    ...
    hello_text = testpage.find_element(locator['LOCATOR_HELLO_TEXT']).text
...
def test_api_user_name(login, user_id):
    ...
    assert response.status_code == 200 and response.json()['username'] == testdata['login']